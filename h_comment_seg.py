...
def comment_seg(cnx):
    ...
    for s_id in target:
                ...
                if (concludsion != None):
                    ...
                    for each in nodes_array:
                        if each[0] == concludsion and each[1] == color:
                            ...
                            break
                    if flag == 0:
                        nodes_array.append([concludsion, color, 1, evaluation])
        ...
        for node in nodes_array:
            ...
            weight = node[2]
            evaluation = node[3]
            add_Data(cnx, idid, word, color, shape,
                     weight, evaluation, hotel_id)
        ...
        for relationship in relationships_array:
            ...
            from_id, to_id = build_relationship(
                cnx, from_seg, to_seg, color, hotel_id)
            print(from_id, to_id, color)
            weight = 1
            add_Relationship(cnx, from_id, to_id, color, weight, hotel_id)
...
def markTopTwo_nodes(cnx, s_id):
    ...
    sql_1 = "SELECT id,weight FROM `h_segment_data` WHERE hotel_id = '" + s_id + \
        "' AND weight >= 2 AND evaluation = 'P' ORDER BY weight DESC LIMIT 2"
    sql_2 = "SELECT id,weight FROM `h_segment_data` WHERE hotel_id = '" + s_id + \
...
def markTopTwo_relationships(cnx, s_id):
    ...
    cursor2 = cnx.cursor(buffered=True)
    cursor3 = cnx.cursor(buffered=True)
    sql_1 = "SELECT from_id,to_id,hotel_id,weight FROM `h_segment_relationship` WHERE hotel_id = '" + \
    ...
    for rec in cursor:
        ...
        new_weight = weight * 6
        print(weight, "-------->", new_weight)
        query2 = ("UPDATE h_segment_relationship"
                  " SET color=%s,weight=%s"
                  " WHERE from_id=%s AND to_id=%s AND hotel_id=%s")
...
def add_Data(cnx, new_id, segment, color, shape, weight, evaluation, hotel_id):
    cursor = cnx.cursor(buffered=True)
    add_data = ("INSERT INTO h_segment_data"
                "(id, segment, color, shape,weight,evaluation ,hotel_id)"
                "VALUES (NULL,%s,%s,%s,%s,%s,%s)")
    data = (segment, color, shape, weight, evaluation, hotel_id)
...
def check_exist(cnx, from_id, to_id, color, hotel_id):
    cursor = cnx.cursor(buffered=True)
    sql = "SELECT * FROM h_segment_relationship WHERE from_id = '" + str(from_id) + \
        "' AND to_id = '" + str(to_id) + "' AND color = '" + \
        color + "' AND hotel_id = '" + hotel_id + "'"
    cursor.execute(sql)
...
def add_Relationship(cnx, from_id, to_id, color, weight, hotel_id):
    # print("正在加入", from_id, to_id)
    if_exist = check_exist(cnx, from_id, to_id, color, hotel_id)
    if (if_exist == True):
        print("重複")
        updateWeight(cnx, from_id, to_id, color, hotel_id)
    else:
        cursor = cnx.cursor()
        # print(from_id,to_id)
        add_relation = ("INSERT INTO `h_segment_relationship`"
                        "(from_id, to_id,color,weight ,hotel_id) "
                        "VALUES (%s,%s,%s,%s,%s)")
        data = (from_id, to_id, color, weight, hotel_id)
...
def updateWeight(cnx, from_id, to_id, color, hotel_id):
    sql = "SELECT weight FROM h_segment_relationship WHERE from_id = '" + str(from_id) + \
        "' AND to_id = '" + str(to_id) + "' AND color = '" + \
        color + "' AND hotel_id = '" + hotel_id + "'"
    cursor = cnx.cursor()
    cursor2 = cnx.cursor()
    ...
    query2 = ("UPDATE h_segment_relationship"
              " SET weight=%s"
              " WHERE hotel_id=%s AND from_id=%s AND to_id=%s AND color=%s")
    data = (weight, hotel_id, from_id, to_id, color)
    cursor2.execute(query2, data)
    cnx.commit()


#bulid relationship (red to red) (green to green) in each comment
def build_relationship(cnx, from_seg, to_seg, color, hotel_id):
