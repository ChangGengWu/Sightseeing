...
def add_Data(new_id, segment, color, shape, site_id):
    cursor = cnx.cursor(buffered=True)
    if_exist = check_exist(segment, color, site_id)
    if (if_exist):
        pass
    else:
...
def check_exist(segment, color, site_id):
    cursor = cnx.cursor(buffered=True)
...
def add_Relationship(from_id, to_id, site_id):
    cursor = cnx.cursor(buffered=True)
...
def build_relationship(keyword_lst, color, site_id):
    for i in range(len(keyword_lst) - 1):
        ...
        for elee in cursor2:
            to_id = elee[0]
        add_Relationship(from_id, to_id,site_id)
...
def get_keyword(compare):
    ...
    adj = ['不是特別好', '不太足夠用心', '態度很差', '一位難求', '態度很好', '非常不好', '不夠清楚', '不夠簡單', '非常難找', '鬼斧神工', '印象深刻', '鬼斧神工', '嘆為觀止', '路途遙遠', '相當不便', '印象深刻', '栩栩如生', '鬼斧神工', '印象深刻', '沒有很多', '態度差', '沒禮貌', '有耐心', '有禮貌', '態度好', '再加強', '不理想', '不建議', '沒品質', '不值得', '最棒的', '特別好', '差太多', '不清楚', '更舒服', '沒耐性', '不方便', '超方便', '不乾淨', '不好吃', '種類多', '美麗的', '漂亮的', '蠻貴的', '不便宜', '不用錢', '很實惠', '有點小', '蠻小的', '小點小', '蠻大的', '有點少', '非常少', '有點多', '多到爆', '非常多', '和善', '不便', '不易', '詭異', '害怕', '危險', '掃興', '差勁', '不推', '糟糕', '良好', '齊全', '推薦', '開心', '喜歡', '好玩', '推薦', '適合', '值得', '複雜', '失望', '錯誤', '不易', '新鮮', '明亮', '寬敞', '優美', '舒服', '舒適', '熱鬧', '平靜', '擁擠', '車多', '讚嘆', '佩服', '厲害', '震撼', '壯觀', '遙遠', '傲慢', '塞車', '不錯', '便利', '骯髒', '整潔', '乾淨', '無聊', '無趣', '精美', '厲害', '有趣', '細膩', '優秀', '豐富', '用心', '驚喜', '很棒', '創意', '不錯', '難吃', '精緻', '大量', '好吃', '難看', '好美', '蠻貴', '超貴', '很貴', '免費', '便宜', '實惠', '平價', '低廉', '合理', '親民', '蠻小', '很小', '窄小', '巨大', '超大', '很大', '超少', '蠻少', '很少', '較少', '蠻多', '超多', '很多', '許多', '慢', '久', '低', '醜', '爛', '酷', '擠', '兇', '髒', '亂', '凍', '寒', '冷', '熱', '曬', '醜', '美', '貴', '小', '大', '少', '多']
    result_noun = ""
    result_adj = ""
    for n in noun:
        if n in compare:
...
for each in cursor_all:
    ...
    for word in keyword_lst:
        shape = "circle"
        idid = ""
        add_Data(idid, word, color, shape, site_id)
    build_relationship(keyword_lst, color, site_id)
